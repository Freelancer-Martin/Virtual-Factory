...
function yourprefix_frontend_form_register() {
	$cmb = new_cmb2_box( array(
		...
		'object_types' => array( 'looking_listing' ),
		'hookup'       => false,
		'save_fields'  => false,
	) );
  $cmb->add_field( array(
    'name'    => __( 'Status', 'YOURTEXTDOMAIN' ),
    'id'      => 'submitted_post_status',
    'type'    => 'text',
    'default' => ! empty( $_POST['submitted_post_status'] )
      ? $_POST['submitted_post_status']
      : __( 'New Post', 'YOURTEXTDOMAIN' ),
  ) );
  $cmb->add_field( array(
		'name'    => __( 'Name', 'YOURTEXTDOMAIN' ),
		'id'      => 'submitted_post_title',
		'type'    => 'text',
		'default' => ! empty( $_POST['submitted_post_title'] )
			? $_POST['submitted_post_title']
			: __( 'Name', 'YOURTEXTDOMAIN' ),
	) );
  $cmb->add_field( array(
    'name'    => __( 'Aaddress', 'YOURTEXTDOMAIN' ),
    'id'      => 'submitted_post_aaddress',
    'type'    => 'text',
    'default' => ! empty( $_POST['submitted_post_aaddress'] )
      ? $_POST['submitted_post_aaddress']
      : __( 'Aaddress', 'YOURTEXTDOMAIN' ),
  ) );
  $cmb->add_field( array(
    'name'    => __( 'Size', 'YOURTEXTDOMAIN' ),
    'id'      => 'submitted_post_size',
    'type'    => 'text',
    'default' => ! empty( $_POST['submitted_post_size'] )
      ? $_POST['submitted_post_size']
      : __( 'Size', 'YOURTEXTDOMAIN' ),
  ) );
  $cmb->add_field( array(
    'name'    => __( 'Building Type', 'YOURTEXTDOMAIN' ),
    'id'      => 'submitted_building_type',
    'type'    => 'text',
    'default' => ! empty( $_POST['submitted_building_title'] )
      ? $_POST['submitted_building_title']
      : __( 'Building Type', 'YOURTEXTDOMAIN' ),
  ) );
	$cmb->add_field( array(
  ...
  $cmb->add_field( array(
    ...
    'type'    => 'multicheck',
  		'options'          => array(
  			'road_desinger' => __( 'Road Designer', 'cmb2' ),
  			'board_member'   => __( 'Board Member', 'cmb2' ),
  			'Investor'     => __( 'Investor', 'cmb2' ),
        'Architect'     => __( 'Architect', 'cmb2' ),
        'sewerge_ngineer'     => __( 'Water Supply and Sewerge Engineer', 'cmb2' ),
        'structural_designer'     => __( 'Structural Designer', 'cmb2' ),
        'supervisor'     => __( 'Supervisor', 'cmb2' ),
  		),
  	) );
...
*/
  ...
  $post_data['post_status']   = $sanitized_values['submitted_post_status'];
	unset( $sanitized_values['submitted_post_status'] );

  $post_data['post_title']   = $sanitized_values['submitted_post_name'];
	unset( $sanitized_values['submitted_post_name'] );

  $post_data['post_aaddress']   = $sanitized_values['submitted_post_aaddress'];
	unset( $sanitized_values['submitted_post_aaddress'] );

  $post_data['post_size']   = $sanitized_values['submitted_post_size'];
	unset( $sanitized_values['submitted_post_size'] );

  $post_data['post_building_type']   = $sanitized_values['submitted_post_building_type'];
	unset( $sanitized_values['submitted_post_building_type'] );
